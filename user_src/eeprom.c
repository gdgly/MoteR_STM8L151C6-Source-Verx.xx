...
void eeprom_sys_load(void){
    ...
    xm[0] = ReadByteEEPROM( addr_eeprom_sys+addr_eeprom_ID_PCS_L );
    xm[1] = ReadByteEEPROM( addr_eeprom_sys+addr_eeprom_ID_PCS_H );
...
void ID_EEPROM_write(void)
{
    ...
    uni_rom_id xn,xd;
     ...
     UnlockFlash( UNLOCK_EEPROM_TYPE );
	WriteByteToFLASH( addr_eeprom_sys+addr_eeprom_ID_PCS_L, xm[1]);
	WriteByteToFLASH( addr_eeprom_sys+addr_eeprom_ID_PCS_H, xm[0]);
     LockFlash( UNLOCK_EEPROM_TYPE );
...
//     if(TIMER_Sensor_close_1s)--TIMER_Sensor_close_1s;
     if(TIME_Receiver_Login_restrict)--TIME_Receiver_Login_restrict;
		 ...
		 if(TIME_Receiver_LED_OUT>0){Receiver_LED_OUT=1;LED_display_page(".  ",0,0,0,0,1);}
                 else {Receiver_LED_OUT=!Receiver_LED_OUT;LED_display_page(".   ",0x80,0,0,500,1);}
             }
